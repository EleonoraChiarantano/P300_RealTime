#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Visual oddball stimuli delivery, with 1 out of 8 events being rare. Square is the frequent stimulus, circle is rare.
...
def start(console_inlet):
    ...
    import pylsl
    from psychopy import visual, core, data, logging, clock
    ...
    # initialize data for polygon generation
    edges_list = [4, 4, 4, 4, 4, 20, 4, 4]
    edges_idx = 0
    ...
    polygon = visual.Polygon(
        ...
        edges=4, size=(0.5, 0.5),
        ori=0.0, pos=(0, 0),
        lineWidth=1.0, colorSpace=color_space, lineColor=stim_color, fillColor=stim_color,
        opacity=None, depth=0.0, interpolate=True, autoLog=True)
    ISI = clock.StaticPeriod(win=win, name='ISI')
    ...
    while not stop:
        ...
        for thisTrial in trials:
                ...
                if msg is not None and msg[0] == "PAUSE":
                    ...
                    while pause:
                        ...
                        if msg is not None and msg[0] == "STOP":
                            ...
                            play = False
                            edges_idx = 0  # restart the indexing of the edges list
                            break
                            # return
                        if msg is not None and msg[0] == "QUIT":
                            # quit the program
            ...
            # pseudo-randomize the polygon edges
            while edges_list[0] == 20:
                shuffle_list(edges_list)
            polygon.edges = edges_list[edges_idx]
            edges_idx += 1
            if edges_idx == 8:
                edges_idx = 0
                shuffle_list(edges_list)
            ...
            routineTimer.add(args.stim_len + args.isi)

            # keep track of which components have finished
            trialComponents = [polygon, ISI]
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                # *polygon* updates
                if polygon.status == NOT_STARTED and tThisFlip >= args.isi - frameTolerance:
                    # keep track of start time/frame for later
                    polygon.frameNStart = frameN  # exact frame index
                    polygon.tStart = t  # local t and not account for scr refresh
                    polygon.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(polygon, 'tStartRefresh')  # time at next scr refresh
                    # push the marker on the lsl stream
                    if polygon.edges == 4:
                        outlet_marker.push_sample('F', globalClock.getTime())
                    if polygon.edges == 20:
                        outlet_marker.push_sample('R', globalClock.getTime())
                    polygon.setAutoDraw(True)
                if polygon.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlip > polygon.tStartRefresh + args.stim_len - frameTolerance:
                ...
                if ISI.status == NOT_STARTED and t >= 0 - frameTolerance:
                    ...
                    ISI.tStop = ISI.tStart + args.isi  # record stop time
